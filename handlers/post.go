...
func (cat *CategoryHandler) CreateCategory(w http.ResponseWriter, r *http.Request) {
	...
	err = cat.repo.Create(context.Background(), category)
	if err != nil {
		cat.logger.Error("unable to create category", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
		// data.ToJSON(&GenericError{Error: err.Error()}, w)
		data.ToJSON(&GenericResponse{Status: false, Message: "Unable to create category. Please try again later"}, w)
		return
	}
